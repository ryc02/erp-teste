...
function bindShellEvents() {
    ...
    if (toggle) {
        toggle.addEventListener('click', () => setSidebarOpen(!container.classList.contains('open')));
    }
    if (backdrop) {
        backdrop.addEventListener('click', () => setSidebarOpen(false));
    }

    // Fechar flyout secundário com Escape
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') closeSidebarSecondary?.();
    });
}
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (state.user) {
                ...
                const initial = (state.user.nome_completo || state.user.username || '?')[0].toUpperCase();
...
/** Atualiza o badge de alertas na topbar. Chame com o número de alertas. */
window.updateAlertBadge = function(count) {
    const badge = document.getElementById('alert-count');
    const btn   = document.getElementById('alert-badge-btn');
    if (!badge || !btn) return;
    ...
    for (const group of order) {
        ...
        html += `
            <div class="nav-item ${isActive ? 'active' : ''}" onclick="openSidebarGroup('${group}')">
                ...
                ${isActive ? '<div class="nav-item-dot"></div>' : ''}
            </div>
        `;
    }
...
window.openSidebarGroup = function(group) {
    ...
    const isAlreadyOpen = secondary?.classList.contains('open') && state.activeSidebarGroup === group;
    ...
    // Auto-navigate to the first module of the group for better UX
    const modulesInGroup = state.menuGroups[group];
    if (modulesInGroup && modulesInGroup.length > 0) {
        const currentMod = window.getAppModule(state.currentView);
        if (!currentMod || currentMod.featureGroup !== group) {
            carregarModulo(modulesInGroup[0].id);
        }
    }
}
...
function renderSidebarSecondary(group) {
    ...
    const backdrop = document.getElementById('secondary-backdrop');
    ...
    // Se tem apenas 1 item e ele já é a view ativa, não precisamos abrir o flyout
    if (modules.length === 1 && state.currentView === modules[0].id) {
        closeSidebarSecondary();
        return;
    }
    ...
    if (backdrop) {
        ...
        if (!backdrop.dataset.bound) {
            ...
            backdrop.addEventListener('click', closeSidebarSecondary);
        }
    }
}
...
window.closeSidebarSecondary = closeSidebarSecondary;
...
function updateActiveModuleUI(moduleId) {
    document.querySelectorAll('#sidebar-secondary [data-module]').forEach(menuItem => {
        menuItem.classList.toggle('active', menuItem.dataset.module === moduleId);
    });
    
    renderSidebarMenu(); // Updates primary menu active dot if needed
    updatePageTitle(window.getAppModule(moduleId));
    closeSidebarSecondary(); // fecha o flyout após navegar
}
...
window.carregarModulo = async function(moduloId) {
    ...
    if (window.UXTelemetry) {
        window.UXTelemetry.trackEvent('MODULE_LOADING', { moduloId });
    }
    ...
    try {
        ...
        if (window.UXTelemetry) {
            window.UXTelemetry.trackEvent('MODULE_LOADED_SUCCESS', { moduloId });
        }
    } catch (e) {
        console.error(`Erro ao carregar módulo ${moduloId}:`, e);

        if (window.UXTelemetry) {
            window.UXTelemetry.trackError('MODULE_LOAD_ERROR', e.message, { moduloId });
        }
...
async function checkDBHealth() {
    ...
    try {
        ...
        if (data.status === 'healthy') {
            if (indicator) indicator.className = 'db-status-dot healthy';
            if (text) text.innerText = 'Banco Online';
        } else {
            if (indicator) indicator.className = 'db-status-dot error';
